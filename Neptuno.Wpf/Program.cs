...
namespace NeptunoLab05
{
    ...
    public sealed class Shell
    {
        ...
        private TextBox connection; private readonly List<CrudView> views = new List<CrudView>();
        ...
        public Shell(string[] args)
        {
            ...
            connection = (TextBox)Window.FindName("ConnectionBox");
            connection.Text = ConfigurationManager.ConnectionStrings["Neptuno"].ConnectionString;
            ((Button)Window.FindName("ConnectButton")).Click += delegate { Connect(); };
            ...
            { if (e.Source == tabs && Db != null) Run(delegate { Refresh(); }); };
        ...
        {
            Run(delegate {
                var candidate = new DataAccess(connection.Text);
                ...
                Refresh(); status.Text = "Conexión activa · Los mantenimientos muestran únicamente registros con Activo = 1.";
            });
        }
    ...
    public static class Ui
    {
        ...
        { var b = new Button { Content = text }; b.Click += delegate { action(); }; return b; }
        public static TextBlock Title(string text)
        { return new TextBlock { Text = text, FontSize = 21, FontWeight = FontWeights.SemiBold, Margin = new Thickness(0,0,0,12) }; }
        public static FrameworkElement Labeled(string label, FrameworkElement input, double width)
        { var p = new StackPanel { Width = width, Margin = new Thickness(0,0,15,10) }; p.Children.Add(new TextBlock { Text = label, Margin = new Thickness(0,0,0,4) }); p.Children.Add(input); return p; }
    ...
    public sealed class CrudView
    {
        ...
        public CrudView(Shell shell, Entity entity)
        {
            ...
            buttons.Children.Add(Ui.Button("Insertar", delegate { shell.Run(delegate { Save(true); }); }));
            buttons.Children.Add(Ui.Button("Actualizar", delegate { shell.Run(delegate { Save(false); }); }));
            buttons.Children.Add(Ui.Button("Eliminar", delegate { shell.Run(Delete); }));
            buttons.Children.Add(Ui.Button("Recargar", delegate { shell.Run(shell.Refresh); })); top.Children.Add(buttons);
        ...
        private void Save(bool insert)
        {
            ...
            shell.Refresh(); shell.Notify(insert ? "Registro insertado. ID generado: "+id : "Registro actualizado correctamente.");
        }
        private void Delete()
        {
            ...
            shell.Refresh(); shell.Notify("Baja lógica realizada: Activo = 0. El registro se conserva en SQL Server.");
        }
    }
    ...
    public sealed class ReportView
    {
        ...
        public ReportView(Shell shell)
        {
            ...
            top.Children.Add(Ui.Button("Consultar reporte",delegate { shell.Run(delegate { shell.RequireDb(); Load(); }); })); top.Children.Add(total); Root.Children.Add(grid);
        }
